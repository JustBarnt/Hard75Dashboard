<script lang="ts">
	...
	//const createAccDispatch = createEventDispatcher();
	let accounts: { username: string, password: string } = { username: "justbarnt", password: "admin" };
	let attempt: { username: string, password: string } = { username: "", password: "" };
	...
	{
		success =
			attempt.username === accounts.username &&
			attempt.password === accounts.password;
	}
...
<style lang="scss">
	...
	.login-field {
		@include input-base;
		box-shadow: 0 0 0.75rem $purpleDark;
